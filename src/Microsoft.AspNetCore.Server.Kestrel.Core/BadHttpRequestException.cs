...
namespace Microsoft.AspNetCore.Server.Kestrel.Core
{
    public sealed class BadHttpRequestException : IOException
    {
        private BadHttpRequestException(string message, int statusCode, RequestRejectionReason reason)
            : this(message, statusCode, null, reason)
        { }

        private BadHttpRequestException(string message, int statusCode, HttpMethod? requiredMethod, RequestRejectionReason reason)
            : base(message)
        {
        ...
        internal static BadHttpRequestException GetException(RequestRejectionReason reason)
        {
            ...
            switch (reason)
            {
                ...
                case RequestRejectionReason.RequestTimeout:
                    ex = new BadHttpRequestException(CoreStrings.BadRequest_RequestTimeout, StatusCodes.Status408RequestTimeout, reason);
                    break;
                case RequestRejectionReason.OptionsMethodRequired:
                    ex = new BadHttpRequestException(CoreStrings.BadRequest_MethodNotAllowed, StatusCodes.Status405MethodNotAllowed, HttpMethod.Options, reason);
                    break;
                case RequestRejectionReason.ConnectMethodRequired:
                    ex = new BadHttpRequestException(CoreStrings.BadRequest_MethodNotAllowed, StatusCodes.Status405MethodNotAllowed, HttpMethod.Connect, reason);
                    break;
